...
|프로젝트 키워드|읽기 오류 분석, 음성 인식(STT), 맞춤형 동화책 생성|
|트랙|산학|
|프로젝트 멤버|정유진, 정윤아, 복지희|
|팀 지도교수|윤명국 교수님|
...
|사용할 소프트웨어 패키지의 명칭과 핵심기능/용도, 사용 시나리오|**[Frontend]** React Native : 모바일 앱 UI/UX 개발, 동화책 뷰어 및 읽기 훈련 인터페이스 제공<br><br> **[Backend]** Spring Boot : REST API 서버 구축, 사용자 관리·학습 데이터 저장<br>FastAPI : STT/LLM 모듈 연동, 적응형 알고리즘 처리<br><br> **[협업 툴]** Git/GitHub : 소스 코드 버전 관리<br>Figma : UI 프로토타입 제작<br>Notion : 문서 및 일정 관리<br>Discord : 팀 커뮤니케이션<br><br> **[AI / API]** Whisper : 아이의 낭독 음성 인식 및 전사<br>KoNLPy/형태소 분석기 : 한국어 문장 난이도 제어, 어휘 분석<br>Stable Diffusion API : 삽화 이미지 생성<br>TTS (Coqui TTS/Piper) : 동화 오디오북 기능 제공<br>LLM(OpenAI GPT 계열) : 아이 수준에 맞는 동화 문장 생성 및 질문 대화 제공<br>Adaptive Learning Engine(직접 구현) : 학습 데이터 기반 난이도 조절 및 피드백|
